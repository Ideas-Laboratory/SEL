...
# 在计算AttrForce之前，调整边的引力系数
@njit(parallel=False, cache=True)
def modified_AttrForce(attr_force, dC, pos, edgesrc, edgetgt, bias, N, a1, alpha, numba_attr_weight):
    ...
    for i in prange(N):
        ...
        for k in range(edgesrc[i], edgesrc[i + 1]):
            ...
            if numba_attr_weight.get(edge_tuple, 1.0) != 1.0:
                force_coefficient = numba_attr_weight[edge_tuple]
            else:
                force_coefficient = 1.0

            R = a1 / (constant + mv0 * mv0 + mv1 * mv1) + b
            tempforce -= force_coefficient * R * mv0
            tempforce1 -= force_coefficient * R * mv1
...
@njit(parallel=False, cache=True)
def ApplyForce_align(dC,super, pos, N):
    # 找出不属于 super 的节点
    non_super_nodes = [i for i in range(N) if i not in super]
    # print(non_super_nodes)
    # 遍历不属于 super 的节点数组
    for i in non_super_nodes:
        mv0 = dC[i][0]
        mv1 = dC[i][1]
        d = np.sqrt(mv0 * mv0 + mv1 * mv1) + 1e-32
        R = min(d, 1.0)
...
@njit(parallel=False, cache=True)
def ApplyRepulsiveCenterForce(pos, centers, ranges, super, N):
    for j in prange(len(centers)):
        center = centers[j]
        R = ranges[j]
        ...
        for i in prange(N):
            ...
            # 检查当前节点是否在 super 中，如果是则跳过
            if i in super:
                continue

            # 计算当前位置与 center 的距离
            distance = current_pos - center
            dist = np.linalg.norm(distance)  # 计算当前位置到中心的距离

            # 只有在距离小于 1.2*R 时才计算斥力
            if dist < 1.2 * R:
                # 计算斥力（与距离成反比）
                repulsive_force = (distance / dist) * (1.2 * R - dist)  # 斥力向量（与距离成反比并推动节点到 1.2*R 位置）

                # 更新当前位置
                pos[i][0] += repulsive_force[0]
                pos[i][1] += repulsive_force[1]

@njit(parallel=False, cache=True)
def ApplyCenterForce(pos, centers, attraction_strength, N):
    for i in prange(N):
        current_pos = pos[i]
        center = centers[i]

        # 计算吸引力向量
        attraction_force = (center - current_pos) * attraction_strength[i]

        # 更新当前位置
        pos[i][0] += attraction_force[0]
        pos[i][1] += attraction_force[1]
